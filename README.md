...
7. **Evaluation scorecard** (`/factory evaluate`, also automatic at the end of Build/Maintain): a **separate adversarial subagent** grades the built product against its design across four axes (requirements met, design conformance, verification strength, risk handling), **every score anchored to evidence** (no self-grading). The verdict is a **"design realization"** (realized/partially-realized/diverged), and each run accumulates a **longitudinal trend** so you can see whether successive changes improved or regressed quality.

See [GUIDE_for_beginners.en.md](GUIDE_for_beginners.en.md) for a plain-language walkthrough, or [GUIDE_advanced.en.md](GUIDE_advanced.en.md) for the design rationale, the 7 hardening rules (anti-rationalization, evidence-based verification, source-driven decisions, etc.), the 4 operational enhancements (adaptive depth, resume, telemetry, Stage 5 hardening), Maintain mode, and the Evaluation scorecard.
